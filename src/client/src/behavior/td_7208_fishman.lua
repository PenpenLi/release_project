tree = {type=2,title='根',childs={
	{type=1,title='有敌人',childs={
		{type=7,title='获取10000目标',func = 'aim_platform_target',args={dis=10000}},
		{type=2,title='攻击或移动',childs={
			{type=1,title='对话',childs={
				{type=7,title='下方节点执行2次',func = 'execute_count', args={n=2}},
				{type=7,title='气泡对话',func = 'bubble_dialogue',args={id = {72081},time =3}}
				}},
			{type=1,title='技能',childs={
				{type=7,title='目标450以内',func = 'target_in_range', args={max=450}},
				{type=7,title='skill',func = 'ai_state', args = {state = 'skill', cd = 8}}
				}},
			{type=1,title='普攻1',childs={
				{type=7,title='目标240以内',func = 'target_in_range', args={max=240}},
				{type=7,title='attack',func = 'ai_state', args = {state = 'attack', cd = 5}}
				}},
			{type=7,title='移动目标240',func = 'move_to_target', args={distance_max = 240}}
			}}
		}}
	}
}