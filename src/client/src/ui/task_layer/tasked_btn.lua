...
tasked_btn = lua_class( 'tasked_btn' )
local load_texture_type = TextureTypePLIST

function tasked_btn:_init( layer )
	self.layer = layer
	self:create_btn( )
	self:init_lbl()
end
...
function tasked_btn:create_btn(  )
	self.btn = self.layer:get_widget('tasked_button'):clone()
	self.btn:setName('hahaha')
	
end

function tasked_btn:get_btn(  )
	return self.btn
end

function tasked_btn:init_lbl(  )
	--任务标题
	...
	self.lbl_receive = self.btn:getChildByName('lbl_receive')
	--self.lbl_receive:setFontName(ui_const.UiLableFontType)
	self.lbl_receive:enableOutline(ui_const.UilableStroke, 1)
...
--设置任务标题
function tasked_btn:set_task_title( t )
	self.lbl_task_title:setString(t)
end

--设置任务内容
function tasked_btn:set_task_cont( t )
	self.lbl_task_cont:setString(t)
end


function tasked_btn:set_target( t )
	self.lbl_target:setString( t )
end

function tasked_btn:set_task_img( route )
	
	self.task_img:loadTexture( route ,load_texture_type)
end


function tasked_btn:set_reward( rwd_data )
...
--保存任务id
function tasked_btn:set_id( idx )
	self.id = idx
end

function tasked_btn:get_id(  )
	return self.id
end