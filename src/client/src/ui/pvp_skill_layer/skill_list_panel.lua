...
local pvp_skill_layer	= import( 'ui.pvp_skill_layer.pvp_skill_layer' )
...
function skill_list_panel:_init( layer )
	...
	local function front_button_event(_, sender, eventtype )
		if pvp_skill_layer.get_touch() == false then
			return
		end
...
function skill_list_panel:update_list_view(  )
	...
	local z_count = 1
	--取出已装备的数据
	local cast_skill = avatar:get_pvp_def_skills()  --装备的技能
	for i = 1 , page_count do
		...
		for j = (i-1)*6+1,temp do
			...
			if skill ~= nil then
				...
				count = count + 1
				local cell = skill_cell.skill_cell( self.layer , data.type )
...
function skill_list_panel:set_cell_hander( v )
		local function selected_event(sender, eventType)
			...
			if v:get_skill_total_type() == EnumSkillTypes.active then
				if v:get_cell_state() ~= true then
						...
						for idx = 1, 3 do
							...
							if sk == nil  then
								...
								model.get_player():load_pvp_def_skill(idx, v:get_skill_idx())
						...
						for idx = 1, 3 do
							...
							if sk ~= nil and sk == v:get_skill_data()  then
								...
								model.get_player():load_pvp_def_skill(idx, nil)
				...
				if v:get_cell_state() ~= true then
						...
						for idx = 4, 6 do
							...
							if sk == nil  then
								...
								--插入选择面板的主动icon
								self.layer:get_skill_display_panel():replace_icon((idx-3) , v:get_skill_data().data.icon ,v:get_skill_pro_type() ,'b',v.level )
								break
							end
						end
				else				
					local avatar = model.get_player()	
						...
						for idx = 4, 6 do
							...
							if sk ~= nil and sk == v:get_skill_data()  then
								...
								--删除选择面板的被动 icon
								self.layer:get_skill_display_panel():delete_icon( idx-3 ,'b')
