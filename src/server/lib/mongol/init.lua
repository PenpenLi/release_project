...
local socket = require "network/async_socket"
...
--local to_bson = function (a) return tostring(bson.encode(a)) end
--local from_bson = bson.decode
...
local function full_collection_name ( self , collection )
	...
	if coll ~= collection or colldb ~= db then
		coll = collection
		colldb = db
		cname = db .. "." .. collection .. "\0"
	end
	return  cname
end
...
local function docmd ( conn , opcode , message ,  reponseTo )
	...
	scheduler.claim_obj(conn.sock._s)
	id = id + 1
